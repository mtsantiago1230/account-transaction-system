...
import { CreateTransactionDto, UpdateTransactionDto, TransactionResponseDto, DateRangeDto } from '../dtos/transaction.dto';
...
export class TransactionController {
  ...
  @Put(':id/process')
  async processTransaction(@Param('id', ParseUUIDPipe) id: string): Promise<TransactionResponseDto> {
    const transaction = await this.processTransactionUseCase.execute(id);
    return this.mapToResponseDto(transaction);
  }

  @Put(':id/cancel')
  async cancelTransaction(@Param('id', ParseUUIDPipe) id: string): Promise<TransactionResponseDto> {
  ...
  async getTransactionsByDateRange(
    ...
    const transactions = await this.getTransactionsByDateRangeUseCase.execute(start, end);
    return transactions.map(this.mapToResponseDto);
  }

  @Get(':id')
  async getTransactionById(@Param('id', ParseUUIDPipe) id: string): Promise<TransactionResponseDto> {
    const transaction = await this.getTransactionUseCase.execute(id);
    return this.mapToResponseDto(transaction);
  }

  @Get('account/:accountId')
  async getTransactionsByAccount(@Param('accountId', ParseUUIDPipe) accountId: string): Promise<TransactionResponseDto[]> {
    const transactions = await this.getTransactionsByAccountUseCase.execute(accountId);
    return transactions.map(this.mapToResponseDto);
  }

  @Get('reference/:reference')
  async getTransactionByReference(@Param('reference') reference: string): Promise<TransactionResponseDto> {
